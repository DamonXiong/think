...
class AuthApi extends AuthController{
    ...
    public function get_id_cate(Request $request){
        ...
        $dataCate = StoreCategory::pidBySidList($data['id']);//根据一级分类获取二级分类
     ...
     * @return \think\response\Json
     */
    public function get_product_list($keyword = '', $cId = '',$sId ='',$priceOrder = '', $salesOrder = '', $news = 0, $first = 0, $limit = 8)
    {
        if($_GET){
            if(isset($_GET['sid'])){$sId=$_GET['sid'];}
            if(isset($_GET['cid'])){ $cId=$_GET['cid'];}
            if(isset($_GET['keyword'])){$keyword=$_GET['keyword'];}
            if(isset($_GET['priceOrder'])){$priceOrder=$_GET['priceOrder'];}
            if(isset($_GET['salesOrder'])){$salesOrder=$_GET['salesOrder'];}
...
//        $limit = $data['limit'];
        ...
        if($list) setView(0,0,$sId,'search',$keyword);
        return JsonService::successful($list);
    }
    /**
     * 购物车
...
//            unset($productAttr[$k]['attr_values']);
        ...
        $data['reply'] = StoreProductReply::getRecProductReply($storeInfo['id'],3);
...
//        $this->userInfo['orderStatusNum'] = StoreOrder::getOrderStatusNum($this->userInfo['uid']);
    ...
    public function unique(){
        ...
        $this->set_cart($productId,$cartNum = 1,$uniqueId);
    }
    /**
     * 加入到购物车
     * @param string $productId
...
//            HookService::afterListen('store_product_set_cart_after',$res,$this->userInfo,false,StoreProductBehavior::class);
     ...
     * @return \think\response\Json
     */
    public function now_buy($productId = '',$cartNum = 1,$uniqueId = '',$combinationId = 0,$secKillId=0){
        if(!$productId || !is_numeric($productId)) return JsonService::fail('参数错误');
        $res = StoreCart::setCart($this->userInfo['uid'],$productId,$cartNum,$uniqueId,'product',1,$combinationId,$secKillId);
        if(!$res) return JsonService::fail(StoreCart::getErrorInfo());
    ...
    public function create_order($key = '')
    {
        ...
        $order = StoreOrder::cacheKeyCreateOrder($this->userInfo['uid'],$key,$addressId,$payType,$useIntegral,$couponId,$mark,$combinationId,$pinkId,$seckill_id);
...
//        exit();
    ...
    public function demo(){
        $baseCrypt = new RoutineBizDataCrypt('wx7bc36cccc15e4be2','MH4hnXZ/YAVplkjusAjtvQ==');
        $errCode = $baseCrypt->decryptData('PMZ1YlSfPt1x81EFfh6z10rlwTLnlU6hu/9bauh9rJorlnLTw9l4GiCSObVgbCKyrWRCx2b4P3c0R4Ztvbb5E7yZZaZZnp8J0ttXvTLqG1qrju1B5RYRvgwbKroRHtMrDO6AgOCMeHkaBUQD1MfRRw==','DimKDNacszEv296C3cITRw==',$data);
        if ($errCode == 0) {
            print($data . "\n");
        } else {
            print($errCode . "\n");
    ...
    */
    ...
    public function set_bargain_help($bargainId = 0,$bargainUserId = 0){
        ...
        if($res) return JsonService::successful('砍价成功',$res);

...
<div class="row">
    <div class="col-sm-12">
        ...
        $bool_upload_num = 0;
        ?>
        {if condition="$tab_id neq 2"}
        <ul class="nav nav-tabs">
            {volist name="config_tab" id="vo"}
                {if condition="$vo['value'] neq 2"}
                    {if condition="$vo['value'] eq $tab_id"}
                        <li class="active"><a data-toggle="tab" href="{:url('index',array('tab_id'=>$vo['value']))}" aria-expanded="false"> {$vo.label}</a>
                        </li>
                    {else/}
                        <li class=""><a data-toggle="tab" href="{:url('index',array('tab_id'=>$vo['value']))}" aria-expanded="true">{$vo.label}</a>
                        </li>
                    {/if}
                {/if}
            {/volist}

...
<div id="store-attr" class="mp-form" v-cloak="">
    <i-Form :label-width="80" style="width: 100%">
        <Form-Item
            ...
            <Row>
                <i-Col span="4">
                <i-Input type="text" v-model="item.value" placeholder="设置规则"></i-Input>
                </i-Col>
                <i-Col span="4" offset="1">
                <i-Button type="ghost" @click="handleRemove(index)">删除规则</i-Button>
                </i-Col>
            </Row>
            <Row>
                <i-Col span="4">
                ...
                <i-Col span="4" offset="1">
                    <i-Button type="ghost" @click="attrAdd(item)">添加属性</i-Button>
                </i-Col>
                <i-Col span="24">
                    <Tag v-for="(attr,k) in item.detail" @on-close="attrRemove(item,k)" :key="attr" :name="attr" closable>{{ attr }}</Tag>
                </i-Col>
            </Row>
        </Form-Item>
        <Form-Item>
            <Row>
                ...
                <i-Col span="2" offset="2">
                    <i-Button type="primary" @click="addGoods(true)">生成</i-Button>
                </i-Col>
            </Row>
        </Form-Item>
        ...
        <template v-if="items[0].value!='' && items[0].detail.length>0 && attrs.length">
            <template v-for="(attr,index) in attrs">
                <Form-Item>
                    <Row>
                        <template v-for="(item,index) in items" v-if="item.value!='' && item.detail.length>0">
                            <i-Col span="3" style="margin-right: 3px">
                                <i-Select style="width: 100%" v-model="attr.detail[item.value]"
                                          :placeholder="'请选择'+item.value">
                                    <i-Option v-for="(attr,key) in item.detail" :value="attr" :key="attr">{{attr}}
                                    </i-Option>
                                </i-Select>
                            </i-Col>
                        </template>
                        <i-Col span="3" style="margin-right: 3px">
                            <i-Input placeholder="请输入金额" v-model="attr.price" style="width: 100%"
                                     :number="true"></i-Input>
                        </i-Col>
                        <i-Col span="3" style="margin-right: 3px">
                            <i-Input placeholder="请输入库存" v-model="attr.sales" style="width: 100%"
                                     :number="true"></i-Input>
                        </i-Col>
            ...
            methods: {
                ...
                checkAttr(){
                    ...
                    this.items.map(function(item){
                        if(!bool) return;
                        if(!item.value){
                            $eb.message('error','请设置规则');
                            bool = false;
                        }else if(!item.detail.length){
                            $eb.message('error','请设置属性');
                            bool = false;
                        }
                    });
                    return bool;

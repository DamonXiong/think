...
//class Index extends WapBasic
{
    public function index()
    {
        ...
        $storePink = StorePink::where('p.add_time','GT',time()-86300)->alias('p')->where('p.status',1)->join('User u','u.uid=p.uid')->field('u.nickname,u.avatar as src,p.add_time')->order('p.add_time asc')->limit(20)->select();
        if($storePink){

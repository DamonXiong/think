...
class Article extends WapBasic {
    ...
    {
        $content = WechatNews::where('status',1)->where('hide',0)->where('id',$id)->order('id desc')->find();
        if(!$content || !$content["status"]) return $this->failed('此文章已经不存在!');
        $content["content"] = Db::name('wechatNewsContent')->where('nid',$content["id"])->value('content');
        //增加浏览次数
        $content["visit"] = $content["visit"] + 1;
        WechatNews::where('id',$id)->update(["visit"=>$content["visit"]]);
